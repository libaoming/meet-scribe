...
完整调用链路图见 [README.md](README.md#full-call-chain)（英文）与 `docs/architecture.md`（中文）。核心原则：触发靠文件落盘、目录即状态机（inbox/processing/archive/error）、单接缝 `bin/pipeline.sh`。

## 换机恢复（models/ 与 venv/ 不入库）

1. **venv**：`uv venv venv --python 3.11 && uv pip install --python venv/bin/python whisperx`
2. **模型**（全部从 **ModelScope** 下载；国内环境 hf-mirror 已失效、HF 直连不通）：
